...
int run_example_sd(void) {
	...
	if (fresult != FR_OK)
		uartWriteString(UART_1, "ERROR!!! in mounting SD CARD...\n\n");
	else
		uartWriteString(UART_1, "SD CARD mounted successfully...\n\n");
	...
	/* Writing text */
	f_puts(
			"This data is from the FILE1.txt. And it was written using ...f_puts... ",
			&fil);
	...
	if (fresult == FR_OK)
		uartWriteString(UART_1, "File1.txt created and the data is written \n");
	...
	if (fresult == FR_OK)
		uartWriteString(UART_1,
				"file2.txt is open and the data is shown below\n");
	...
	if (fresult == FR_OK)
		uartWriteString(UART_1, "About to update the file2.txt\n");
	...
	if (fresult == FR_OK)
		uartWriteString(UART_1, "Below is the data from updated file2.txt\n");
	uartWriteString(UART_1, buffer);
	uartWriteString(UART_1, "\n\n");
